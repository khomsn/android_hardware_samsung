...
Return<void> RadioImpl::iccOpenLogicalChannel(int32_t serial, const hidl_string& aid, int32_t p2) {
    ...
    if (s_vendorFunctions->version < 15) {
        RLOGD("iccOpenLogicalChannel: vesion < 15");
        ...
        if (pRI == NULL) {
            RLOGD("iccOpenLogicalChannel:pRI == NULL");
            return Void();
        }
        ...
        if (!copyHidlStringToRil(&params.aidPtr, aid, pRI)) {
            RLOGD("iccOpenLogicalChannel: !copyHidl");
            return Void();
        }
...
int radio::getSignalStrengthResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        SignalStrength signalStrength = {};
        if (response == NULL)
            RLOGE("getSignalStrengthResponse: Invalid response = NULL");
...
int radio::getVoiceRegistrationStateResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
            ...
            if (numStrings != 15) {
                RLOGE("getVoiceRegistrationStateResponse Invalid response: numStrings = %i", numStrings);
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
            ...
            if (responseLen != sizeof(RIL_VoiceRegistrationStateResponse)) {
                RLOGE("getVoiceRegistrationStateResponse Invalid response: s_vendorFunctions->version = %i", s_vendorFunctions->version);
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
...
int radio::getDataRegistrationStateResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
            ...
            int numStrings = responseLen / sizeof(char *);
            RLOGD("getDataRegistrationStateResponse : responseLen = %i", responseLen);
            RLOGD("getDataRegistrationStateResponse : numStrings = %i", numStrings);
#ifdef SS_EXYNOS_5420
            numStrings -= 1;
#endif
            if ((numStrings != 6) && (numStrings != 11) && (numStrings != 13)) {
                RLOGE("getDataRegistrationStateResponse Invalid response: numStrings = %i", numStrings);
                char **resp = (char **) response;
                for (int i = 0; i < numStrings;i++){
                 RLOGD("getDataRegistrationStateResponse : resp[%i] = %s", i , resp[i]);
                }
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
                char **resp = (char **) response;
            ...
            if (responseLen != sizeof(RIL_DataRegistrationStateResponse)) {
                RLOGE("getDataRegistrationStateResponse Invalid response: responseLen = %i",responseLen);
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
...
#if defined(SS_EXYNOS_5420)
        if (response == NULL || numStrings != 4) {
#else
        if (response == NULL || numStrings != 3) {
#endif
            if(response == NULL) RLOGE("getOperatorResponse -> response: NULL");
            else RLOGE("getOperatorResponse -> numStrings = %i",numStrings);
...
SendSmsResult makeSendSmsResult(RadioResponseInfo& responseInfo, int serial, int responseType,
    ...
    if (response == NULL || responseLen != sizeof(RIL_SMS_Response)) {
        ...
        RLOGE("Invalid response: responseLen = %i", responseLen);
        RLOGE("Invalid response: RIL_SMS_Response = %i", sizeof(RIL_SMS_Response));
        if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
...
/*
int radio::setWideBandAMRStateResponse(int slotId,
                                int responseType, int serial, RIL_Errno e,
                                void *response, size_t responseLen) {
#if VDBG
    RLOGD("setWideBandAMRStateResponse: serial %d", serial);
#endif
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        int ret = responseInt(responseInfo, serial, responseType, e, response, responseLen);
        Return<void> retStatus
                = radioService[slotId]->mRadioResponse->setWideBandAMRStateResponse(responseInfo, ret);
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("setWideBandAMRStateResponse: radioService[%d]->mRadioResponse == NULL", slotId);
    }
...
/*
int radio::broadcastAMCommand(int slotId,
                                  int responseType, int serial, RIL_Errno e,
                                  void *response, size_t responseLen) {
#if VDBG
    RLOGD("broadcastAMCommand: serial %d", serial);
#endif
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        populateResponseInfo(responseInfo, serial, responseType, e);
        Return<void> retStatus
                = radioService[slotId]->mRadioResponse->broadcastAMCommand(responseInfo,
                convertCharPtrToHidlString((char *) response));
        radioService[slotId]->checkReturnStatus(retStatus);
    } else {
        RLOGE("broadcastAMCommand: radioService[%d]->mRadioResponse == NULL", slotId);
    }

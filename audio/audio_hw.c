...
#define LOG_TAG "audio_hw_primary_V27.5"
#define LOG_NDEBUG 0
...
static const char * const use_case_table[AUDIO_USECASE_MAX] = {
    ...
    [USECASE_VOIP_CALL] = "voip-call",
};

#define STRING_TO_ENUM(string) { #string, string }

static int audio_device_ref_count;
...
static int amplifier_open(void)
{
    ...
    if (rc) {
        ...
        ALOGVV("%s: Failed to obtain reference to amplifier module: %s\n",
    ...
    if (rc) {
        ALOGVV("%s: Failed to open amplifier hardware device: %s\n",
                __func__, strerror(-rc));
        return -ENODEV;
    }
...
static const char * const device_table[SND_DEVICE_MAX] = {
    ...
    [SND_DEVICE_OUT_VOICE_EARPIECE] = "voice-call-earpiece",
    [SND_DEVICE_OUT_VOICE_EARPIECE_WB] = "voice-call-earpiece-wb",
    [SND_DEVICE_OUT_VOICE_SPEAKER] = "voice-call-speaker",
    [SND_DEVICE_OUT_VOICE_SPEAKER_WB] = "voice-call-speaker-wb",
    [SND_DEVICE_OUT_VOICE_HEADPHONES] = "voice-call-headphones",
    [SND_DEVICE_OUT_VOICE_HEADPHONES_WB] = "voice-call-headphones-wb",
    [SND_DEVICE_OUT_VOICE_BT_SCO] = "voice-call-bt-sco-headset",
    ...
    [SND_DEVICE_IN_VOICE_MIC] = "voice-call-mic",
    [SND_DEVICE_IN_VOICE_EARPIECE_MIC] = "voice-call-earpiece-mic",
    [SND_DEVICE_IN_VOICE_EARPIECE_MIC_WB] = "voice-call-earpiece-mic-wb",
    [SND_DEVICE_IN_VOICE_SPEAKER_MIC] = "voice-call-speaker-mic",
    [SND_DEVICE_IN_VOICE_SPEAKER_MIC_WB] = "voice-call-speaker-mic-wb",
    [SND_DEVICE_IN_VOICE_HEADSET_MIC] = "voice-call-headset-mic",
    [SND_DEVICE_IN_VOICE_HEADSET_MIC_WB] = "voice-call-headset-mic-wb",
    [SND_DEVICE_IN_VOICE_BT_SCO_MIC] = "voice-call-bt-sco-mic",
    [SND_DEVICE_IN_VOICE_BT_SCO_MIC_WB] = "voice-call-bt-sco-mic-wb",
...
static int set_voice_volume_l(struct audio_device *adev, int volume)
{
    int err = 0;
    /*volume step from 1 to 100*/
...
static snd_device_t get_output_snd_device(struct audio_device *adev, audio_devices_t devices)
{
    ...
    ALOGVV("%s: enter: output devices(%#x), mode(%d)", __func__, devices, mode);
    if (devices == AUDIO_DEVICE_NONE ||
        devices & AUDIO_DEVICE_BIT_IN) {
        ALOGVV("%s: Invalid output devices (%#x)", __func__, devices);
...
exit:
    ALOGVV("%s: exit: snd_device(%s)", __func__, device_table[snd_device]);
    return snd_device;
}
...
static snd_device_t get_input_snd_device(struct audio_device *adev, audio_devices_t out_device)
{
    ...
    usecase = get_usecase_from_type(adev, PCM_CAPTURE|VOICE_CALL|VOICE_COMMUNICATION);
    if (usecase != NULL) {
    ...
    ALOGVV("%s: enter: Audio Mode: (%#x) , out_device(%#x) , in_device(%#x)",
          __func__, mode, out_device, in_device);
    if (mode == AUDIO_MODE_IN_CALL) {
...
exit:
    ALOGVV("%s: exit: in_snd_device(%s)", __func__, device_table[snd_device]);
    return snd_device;
}
                             ...
                             snd_device_t snd_device)
{
    ...
    if (snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES) {
        ALOGVV("Request to enable combo device: enable individual devices\n");
    ...
    if (adev->snd_dev_ref_cnt[snd_device] == 1) {
        ALOGVV("%s: snd_device(%d: %s) > has been activated! Re-New!",
              __func__, snd_device, snd_device_name);
        adev->snd_dev_ref_cnt[snd_device]--;
    }
    adev->snd_dev_ref_cnt[snd_device]++;
    ALOGVV("%s: snd_device(%d: %s)", __func__,
    ...
    ALOGVV("%s: Now snd_dev_ref_cnt[%d: %s] = %d",
              __func__, snd_device, snd_device_name, adev->snd_dev_ref_cnt[snd_device]);
                              ...
                              snd_device_t snd_device)
{
    ...
    if (snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES) {
        ALOGVV("Request to disable combo device: disable individual devices\n");
    ...
    if (adev->snd_dev_ref_cnt[snd_device] == 0) {
        ALOGVV("%s: Already deactivated!", __func__);
        goto exit_from_here;
    }
    adev->snd_dev_ref_cnt[snd_device]--;
    if (adev->snd_dev_ref_cnt[snd_device] == 0) {
                                              ...
                                              snd_device_t snd_device)
{
    ...
    list_for_each(node, &adev->usecase_list) {
        ...
        if (usecase_snd_device != snd_device && (usecase == uc_info || usecase->type == type)) {
            ALOGV("%s: Usecase (%s) is active on (%s) - switching needed ..",
    ...
    if (need_switch) {
        ALOGVV("%s: Switch devices",__func__);
        list_for_each(node, &adev->usecase_list) {
            usecase = node_to_item(node, struct audio_usecase, adev_list_node);
            usecase_snd_device = (type == PCM_PLAYBACK) ? usecase->out_snd_device :
                                  usecase->in_snd_device;
            if (switch_device[usecase->id]) {
            ALOGV("%s: Switch device for Usecase (%s)",
                  __func__, use_case_table[usecase->id]);
                if (usecase_snd_device != SND_DEVICE_NONE ) /* disable all usecase */
                          ...
                          audio_usecase_t uc_id)
{
    ...
    ALOGVV("%s: usecase(%d)", __func__, uc_id);
     ...
     * usecase.
     */
    ALOGVV("%s: Now usecase is (%d):1.PCM_PLAYBACK, 2.PCM_CAPTURE, 4.VOICE_CALL, 7.VOICE_COMMUNICATION, 10.PCM_CAPTURE_LOW_LATENCY", __func__, usecase->type);
    if (usecase->type != VOICE_CALL && adev->voice.in_call) {
        vc_usecase = get_usecase_from_id(adev, USECASE_VOICE_CALL);
        ALOGVV("%s: In Voice call", __func__);
        ...
        ALOGVV("%s: Select VOICE_CALL", __func__);
        ...
        if (adev->bt_sco_ready && adev->out_streaming) {
            ALOGVV("%s: Select VOICE_COMMUNICATION on BT", __func__);
            usecase->devices = AUDIO_DEVICE_OUT_ALL_SCO;
        } else {
            ALOGVV("%s: Select VOICE_COMMUNICATION", __func__);
            usecase->devices = active_out->devices;
        ...
        ALOGVV("%s: Select PCM_PLAYBACK", __func__);
        usecase->devices = active_out->devices;
        in_snd_device = SND_DEVICE_NONE; /*initial input*/
        if (out_snd_device == SND_DEVICE_NONE) {
            ALOGVV("%s: Select PCM_PLAYBACK GET OUTPUT when SND_DEVICE_NONE", __func__);
            out_snd_device = get_output_snd_device(adev, active_out->devices);
        }
    } else if (usecase->type == PCM_CAPTURE) {
        ALOGVV("%s: Select PCM_CAPTURE", __func__);
        usecase->devices = ((struct stream_in *)usecase->stream)->devices;
        out_snd_device = SND_DEVICE_NONE; /*initial output*/
        if (in_snd_device == SND_DEVICE_NONE) {
            ALOGVV("%s: Select PCM_CAPTURE for normal usecase", __func__);
            in_snd_device = get_input_snd_device(adev, AUDIO_DEVICE_NONE);
        }
    }

    ALOGVV("%s: usecase->out_snd_device(%d: %s) out_snd_device(%d: %s), usecase->in_snd_device(%d: %s) in_snd_device(%d: %s)", __func__,
          usecase->out_snd_device, get_snd_device_display_name(usecase->out_snd_device), out_snd_device, get_snd_device_display_name(out_snd_device),
          usecase->in_snd_device,  get_snd_device_display_name(usecase->in_snd_device), in_snd_device,  get_snd_device_display_name(in_snd_device));

    if (out_snd_device == usecase->out_snd_device &&
        in_snd_device == usecase->in_snd_device) {
        ALOGVV("%s: NO CHANGE", __func__);
        return 0;
    }
    ...
    /* Disable current sound devices and 
     * Enable new sound devices
     */
    if (out_snd_device != usecase->out_snd_device && usecase->type != PCM_CAPTURE) {
        check_and_route_usecases(adev, usecase, PCM_PLAYBACK, out_snd_device);
    }

    if (in_snd_device != usecase->in_snd_device && usecase->type != PCM_PLAYBACK) {
...
/* always called with adev lock held */
static void stop_voip_call(struct audio_device *adev)
{
    struct audio_usecase *uc_info;
    ...
    if (adev->bt_sco_inuse) {
        /*if BT SCO is in used stop it and
...
#if 0        
        char bt_rfkill_path[PATH_MAX];
        FILE* btkp_file;
        sprintf(bt_rfkill_path, "/sys/class/rfkill/rfkill0/state");

        if (access(bt_rfkill_path, F_OK) == -1) {
            ALOGE("%s: Failed to load bluetooth rfkill state paths from %s",__func__, bt_rfkill_path);
        } else {
            // with file acccess as write-plus mode 
            btkp_file = fopen(bt_rfkill_path, "w+"); 
            // adds content to the file 
            fprintf(btkp_file, "%i", 0); 
            // closes the file pointed by btkp_file 
            fclose(btkp_file); 
            ALOGE("%s: rfkill block bluetooth", __func__);
            usleep(100000);
            
            // with file acccess as write-plus mode 
            btkp_file = fopen(bt_rfkill_path, "w+"); 
            // adds content to the file 
            fprintf(btkp_file, "%i", 1); 
            // closes the file pointed by btkp_file 
            fclose(btkp_file); 
            ALOGE("%s: rfkill unblock bluetooth", __func__);
        }
#endif
    }
   
    uc_info = get_usecase_from_id(adev, USECASE_VOIP_CALL);
    if (uc_info == NULL) {
        ALOGE("%s: Could not find the usecase (%d) in the list",
              __func__, USECASE_VOIP_CALL);
        return;
    }
...
/* always called with adev lock held */
static int start_voip_call(struct audio_device *adev)
{
    struct audio_usecase *uc_info;
    int ret = 0;
    ...
    /*
     * Separate VOIP usecase out of Normal Playback and Capture usecase 
    ...
    uc_info->id = USECASE_VOIP_CALL;
    uc_info->type = VOICE_COMMUNICATION;
                              ...
                              struct echo_reference_buffer *buffer)
{
    ...
    if (in->devices & (AUDIO_DEVICE_IN_BUILTIN_MIC | AUDIO_DEVICE_IN_BACK_MIC)) {
        /* delay of capture for SPEAKER out put to built in microphone for Tablet add the Air delay too. */
        delay_ns = kernel_delay + buf_delay + rsmp_delay + SSD_SPKTOMIC_DELAY;
    } else {
                          ...
                          struct echo_reference_itfe *reference)
{
    ...
    if (adev->echo_reference != NULL) {
         ...
         * for voip communication use cases */
        adev->echo_reference = NULL;
        release_echo_reference(reference);
        ALOGVV("release_echo_reference");
    }
}
                                                      ...
                                                      uint32_t sampling_rate)
{
    ...
    if (adev->primary_output!= NULL && !adev->primary_output->standby) {
        ...
        ALOGVV("Calling create_echo_reference");
                       ...
                       struct echo_reference_buffer *buffer)
{
    ...
    if (status < 0) {
        ...
        buffer->time_stamp.tv_nsec = 0;
        buffer->delay_ns           = 0;
        ALOGVV("get_playback_delay(): pcm_get_htimestamp error,"
...
static int in_configure_reverse(struct stream_in *in)
{
    ...
    int i;
    ALOGVV("%s: enter: in->num_preprocessors(%d)", __func__, in->num_preprocessors);
    if (in->num_preprocessors > 0) {
        ...
        for (i = 0; i < in->num_preprocessors; i++)
        {
            ...
            fct_status = (*(in->preprocessors[i].effect_itfe))->command(
                                                        ...
                                                        &size,
                                                        &cmd_status);
            ALOGVV("%s: calling EFFECT_CMD_SET_CONFIG_REVERSE",__func__);
                                                 ...
                                                 struct effect_info_s *effect_info)
{
    ...
    if (cmd_status != 0) {
        ALOGVV("in_read_audio_effect_channel_configs(): "
...
static uint32_t in_get_aux_channels(struct stream_in *in)
{
    ...
    ALOGVV("in_get_aux_channels(): return %04x", new_chcfg.aux_channels);
                                        ...
                                        channel_config_t *channel_config)
{
    ...
    ALOGVV("in_configure_effect_channels(): configure effect with channels: [%04x][%04x]",
...
static int in_reconfigure_channels(struct stream_in *in,
    ...
    ALOGVV("in_reconfigure_channels(): config_changed %d effect %p",
          config_changed, effect);

    /* if config changed, reconfigure all previously added effects */
    if (config_changed) {
        int i;
        ALOGVV("%s: config_changed (%d)", __func__, config_changed);
        for (i = 0; i < in->num_preprocessors; i++)
        {
            int cur_status = in_configure_effect_channels(in->preprocessors[i].effect_itfe,
                                                  channel_config);
            ALOGVV("%s: in_configure_effect_channels i=(%d), [main_channel,aux_channel]=[%d|%d], status=%d",
                          __func__, i, channel_config->main_channels, channel_config->aux_channels, cur_status);
            if (cur_status != 0) {
                ALOGVV("in_reconfigure_channels(): error %d configuring effect "
                                   ...
                                   effect_handle_t effect)
{
    ...
    if (status != 0) {
        ALOGVV("in_update_aux_channels(): in_reconfigure_channels error %d", status);
        /* resetting aux channels configuration */
        aux_channels = 0;
        channel_config.aux_channels = 0;
        in_reconfigure_channels(in, effect, &channel_config, true);
    }
    ALOGVV("%s: aux_channels=%d, in->aux_channels_changed=%d", __func__, aux_channels, in->aux_channels_changed);
...
static int in_close_pcm_devices(struct stream_in *in)
{
    ...
    list_for_each(node, &in->pcm_dev_list) {
        ...
        if (pcm_device->pcm) {
            ...
            ALOGVV("%s: : adev->in_pcm_opened = false", __func__);
        }
    }
    ALOGVV("%s: exit", __func__);
    return 0;
...
static int in_open_pcm_devices(struct stream_in *in)
{
    struct pcm_device *pcm_device;
    struct listnode *node;
    struct audio_device *adev = in->dev;
    ...
    list_for_each(node, &in->pcm_dev_list) {
        ...
        ALOGVV("%s: Opening PCM device card_id(%d) device_id(%d)",
    ...
    adev->in_pcm_opened = true;
    ALOGVV("%s: Exit : adev->in_pcm_opened = true", __func__);
    return ret;
...
static int stop_input_stream(struct stream_in *in)
{
    ...
    ALOGVV("%s: enter: usecase(%d: %s)", __func__,
    ...
    /* Input stream is now stopped
     * and device is disabled 
     * this means input is in standby mode
     */
    in->standby = true;
    in->stream_opened = false;
    adev->in_streaming = false;
    ALOGVV("%s: in->standby = true : in->stream_opened = false", __func__);
/*
...
{
    /* Enable input device and stream routing controls */
    int ret = 0;
    ...
    ALOGVV("%s: enter: usecase(%d)", __func__, in->usecase);
     ...
     */
    ALOGVV("%s: Opening PCM device card_id(%d) device_id(%d), channels %d, smp rate %d format %d, \
    ...
    ret = in_open_pcm_devices(in);
    if (ret != 0) goto error_open;
    ...
    /* Input stream is started and 
     * device is enabled and pcm deviced is opened
     * so input is not in standby mode
     */
    in->standby = false;
    in->stream_opened = true;
    adev->in_streaming = true;
    ALOGVV("%s: in->standby = false : in->stream_opened = true", __func__);
    ALOGVV("%s: exit", __func__);
    return ret;
...
error_config:
    ALOGVV("%s: exit: status(%d)", __func__, ret);
    adev->active_input = NULL;
    return ret;
}
...
static int out_close_pcm_devices(struct stream_out *out)
{
    ...
    list_for_each(node, &out->pcm_dev_list) {
        ...
        if (pcm_device->pcm) {
            ...
            ALOGVV("%s: exit : adev->out_pcm_opened = false", __func__);
        }
    }
    ALOGVV("%s: exit", __func__);
...
static int out_open_pcm_devices(struct stream_out *out)
{
    ...
    list_for_each(node, &out->pcm_dev_list) {
        ...
        adev->out_pcm_opened = true;
        ALOGVV("%s: exit : adev->out_pcm_opened = true", __func__);
    }
    return ret;
...
static int stop_output_stream(struct stream_out *out)
{
     ...
     * ************************************************************/
    out->standby = true;
    ALOGVV("%s: out->standby = true", __func__);
...
static int start_output_stream(struct stream_out *out)
{
    ...
    ALOGVV("%s: enter: usecase(%d: %s) devices(%#x) channels(%d)",
          __func__, out->usecase, use_case_table[out->usecase], out->devices, out->config.channels);

    /*************** Out Stream Started ***********
     * output path is enabled and out pcm device
     * is opened;
     * So output not STANDBY
     * *******************************************/
    out->standby = false;
    ALOGVV("%s: out->standby = false", __func__);
    adev->out_streaming = true;
...
static int do_out_standby_l(struct stream_out *out)
{
    ...
    if (out->usecase != USECASE_AUDIO_PLAYBACK_OFFLOAD) {
        ...
        if (out->echo_reference != NULL) {
            ...
            if (out->echo_reference_generation != adev->echo_reference_generation) {
                ALOGVV("%s: release_echo_reference %p", __func__, out->echo_reference);
    ...
    ALOGVV("%s:Exit: output is not in used right now!", __func__);

    return status;
}

/** output stream can standby when mode in_call active, but do not disable output paths **/
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGVV("%s: PREPROCESSING_ENABLED!",__func__);
#endif

    ALOGVV("%s: enter: usecase(%d: %s) kvpairs: %s out->devices(%#x) "
          "adev->mode(%#x)",
    ...
    if (ret >= 0) {
        val = atoi(value);
        if (!adev->bt_sco_ready && (pvod & AUDIO_DEVICE_OUT_ALL_SCO)) {
            if(val == AUDIO_DEVICE_OUT_ALL_SCO)
            val = AUDIO_DEVICE_OUT_SPEAKER;
        }

        ALOGVV("%s: routing: usecase(%d: %s) devices=(%#x) adev->mode(%#x)",
        ...
        if (val != SND_DEVICE_NONE) {
            ...
            if ((pvod & val) == 0) {
                if (adev->mode == AUDIO_MODE_IN_CALL || adev->mode == AUDIO_MODE_IN_COMMUNICATION || adev->voip) {
                    ALOGVV("%s: Audio path changed!",__func__);
                    adev->path_changed = true;
                }
            }

            if (!out->standby && adev->mode != AUDIO_MODE_IN_CALL) {
                ALOGVV("%s: Now OUT NOT STANDBY", __func__);
                uc_info = get_usecase_from_id(adev, out->usecase);
                if (uc_info == NULL) {
                    ALOGE("%s: Could not find the usecase (%d) in the list",
                          __func__, out->usecase);
             ...
             * and how sound modem performed
             * 
             * ******************************************************************************/
            
            if ((adev->mode == AUDIO_MODE_IN_CALL) && adev->voice.in_call && adev->path_changed) {
                ALOGVV("%s: CHANGE DEVICES FOR VOICE CALL!",__func__);
                if ((out->devices & AUDIO_DEVICE_OUT_ALL_SCO) && !adev->bt_sco_inuse) {
                    ...
                    */
                    ...
                    ALOGVV("%s: BLUETOOTH ACTIVE & Ready!",__func__);
                    ALOGVV("%s: CHANGE DEVICES FOR VOICE CALL to BLUETOOTH!",__func__);
                    start_voice_session_bt_sco(adev);
                    /* 4 Turn on / Start voice pcm */
                    start_voice_session(adev->voice.session);
                } else {
                    /* 1. Stop voice pcm */
                    stop_voice_session(adev->voice.session);
            ...
            } else if ((adev->mode == AUDIO_MODE_IN_COMMUNICATION || adev->voip) && adev->path_changed && !out->standby) {
                ALOGVV("%s: CHANGE DEVICES FOR VOIP CALL!",__func__);
                if ((out->devices & AUDIO_DEVICE_OUT_ALL_SCO)
                    && adev->bt_sco_ready && !adev->bt_sco_inuse) {
                    ALOGVV("%s: BLUETOOTH ACTIVE!",__func__);
                    /* 1. Route stream to new device */
                    ALOGVV("%s: CHANGE DEVICES FOR VOIP CALL to BLUETOOTH!",__func__);
                    //select_devices(adev, USECASE_VOIP_CALL);
                    /* 2. Turn on / Start SCO pcm then Voice pcm*/
                    ALOGVV("%s: Start VOIP CALL over BLUETOOTH!",__func__);
                    start_voice_session_bt_sco(adev);
                }
                adev->path_changed = false;
#if 0                
    ...
    if (ret > 0)
        ret = 0;
    ALOGVV("%s: exit: code(%d)", __func__, ret);
    return ret;
}
...
static char* out_get_parameters(const struct audio_stream *stream, const char *keys)
{
    ...
    int ret;
    bool first = true;
    ALOGVV("%s: enter: keys - %s", __func__, keys);
    ...
    ALOGVV("%s: exit: returns - %s", __func__, str);
    return str;
}
                         ...
                         size_t bytes)
{
    ...
    if (out->standby) {
        ...
        if (adev->active_input &&
            (adev->active_input->source == AUDIO_SOURCE_VOICE_COMMUNICATION ||
                adev->active_input->source == AUDIO_SOURCE_MIC)) {
                    in = adev->active_input;
                    ALOGVV("%s: enter:) force_input_standby true", __func__);
        }
#endif
        pthread_mutex_unlock(&adev->lock);
#if PREPROCESSING_ENABLED
    ...
    if (out->usecase == USECASE_AUDIO_PLAYBACK_OFFLOAD) {
        ...
        if (android_atomic_acquire_load(&adev->echo_reference_generation)
            ...
            out->echo_reference_generation = adev->echo_reference_generation;
            out->echo_reference = adev->echo_reference;
            ALOGVV("%s: update echo reference generation %d", __func__,
                  out->echo_reference_generation);
            pthread_mutex_unlock(&adev->lock);
        }
#endif
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    ALOGVV("%s: enter: kvpairs=%s", __func__, kvpairs);
    parms = str_parms_create_str(kvpairs);
...
exit:
    ...
    if (read_and_process_successful == false) {
        ...
        ALOGVV("%s: read failed - sleeping for buffer duration", __func__);
        struct timespec t = { .tv_sec = 0, .tv_nsec = 0 };
        clock_gettime(CLOCK_MONOTONIC, &t);
        const int64_t now = (t.tv_sec * 1000000000LL + t.tv_nsec) / 1000;
    ...
    if ( enable == true ) {
        ...
        for (i=0; i < in->num_preprocessors; i++) {
            if (status == 0) { /* status == 0 means an effect was removed from a previous slot */
                ...
                ALOGVV("add_remove_audio_effect moving fx from %d to %d", i, i-1);
                continue;
            }
            if ( in->preprocessors[i].effect_itfe == effect ) {
                ALOGVV("add_remove_audio_effect found fx at index %d", i);
        ...
        ALOGVV("%s: enable(%d), in->aux_channels_changed(%d)", __func__, enable, in->aux_channels_changed);
    }
    ALOGI("%s:  num_preprocessors = %d", __func__, in->num_preprocessors);

    if ( memcmp(&desc.type, FX_IID_AEC, sizeof(effect_uuid_t)) == 0) {
        in->enable_aec = enable;
        ALOGVV("add_remove_audio_effect(), FX_IID_AEC, enable: %d", enable);
        if (!in->standby) {
            select_devices(in->dev, in->usecase);
            do_in_standby_l(in);
        }
...
static int in_add_audio_effect(const struct audio_stream *stream,
                               effect_handle_t effect)
{
    ALOGVV("%s: effect %p", __func__, effect);
                                   ...
                                   const char *address __unused)
{
    ...
    ALOGVV("%s: enter: sample_rate(%d) channel_mask(%#x) devices(%#x) flags(%#x)",
    ...
    *stream_out = NULL;
    ...
    if (out->flags & AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD) {
        ...
        ALOGVV("%s: offloaded output offload_info version %04x bit rate %d",
        ...
        ALOGVV("%s: use AUDIO_PLAYBACK_DEEP_BUFFER",__func__);
    } else {
        out->usecase = USECASE_AUDIO_PLAYBACK;
        out->sample_rate = out->config.rate;
    }
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ALOGVV("%s: enter: %s", __func__, kvpairs);
    parms = str_parms_create_str(kvpairs);

    ret = str_parms_get_str(parms, "restarting", value, sizeof(value));
    if (ret >= 0) {
        if (strcmp(value, "true") == 0) {
            adev->adev_restart = true;
            ALOGVV("%s:: Hal Device Restarting", __func__);
            if(!adev->voice.bluetooth_wb)
                adev->voice.bluetooth_wb = true;
        } else {
            adev->adev_restart = false;
        }
    }
    ...
    if (ret >= 0) {
        adev->bt_hs_ready = true;
        ALOGVV("%s:: BLUETOOTH Headset is connected", __func__);
    }
    ...
    if (ret >=0) {
        if (strcmp(value, "false") == 0) {
            ...
            ALOGVV("%s:: BT_A2dp is active", __func__);
        } else {
            adev->bt_a2dp_spd = true;
            if(adev->bt_sco_active && adev->bt_hs_ready) {
                adev->bt_sco_ready = true;
            }
        }
    }
    ...
    if (ret >=0) {
        if (strcmp(value, "on") == 0) {
            ...
            ALOGVV("%s:: BT_SCO is activated", __func__);
            if(adev->bt_hs_ready){
                adev->bt_sco_ready = true;
            }
        } else {
            ...
            adev->bt_hs_ready = false;
            adev->bt_sco_ready = false;
            ALOGVV("%s:: BT_SCO is deactivated", __func__);
        }
    }
    ...
    if (ret >= 0) {
        if (strcmp(value, AUDIO_PARAMETER_VALUE_ON) == 0) {
            ...
            ALOGVV("%s: enter: adev->screen_off = false", __func__);
        } else { 
            adev->screen_off = true;
            ALOGVV("%s: enter: adev->screen_off = true", __func__);
        }
    }
    ...
    ALOGVV("%s: exit with code(%d)", __func__, ret);
    return ret;
}
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ...
    pthread_mutex_lock(&adev->lock);
    /* cache volume */
    adev->voice.volume = (int)(volume*100);
    ALOGVV("%s: adev->voice.volume = %d", __func__, adev->voice.volume);
...
static int adev_set_mode(struct audio_hw_device *dev, audio_mode_t mode)
{
    ...
    if (adev->mode != mode) {
        ...
        if (mode == AUDIO_MODE_IN_CALL) {
            ALOGVV("%s: START VOICE CALL!",__func__);
            start_voice_call(adev);
        } else if (mode == AUDIO_MODE_IN_COMMUNICATION) {
            if(!adev->voip){
                ALOGVV("%s: START VOIP CALL!",__func__);
                start_voip_call(adev);
            }
        }
    }
                                  ...
                                  audio_source_t source)
{
    ...
    *stream_in = NULL;
    ...
    if (in->stream_opened) {
        ALOGVV("%s: Input stream is opened. Close First!", __func__);
        in_standby_l(in);
        goto stream_opened;
    }
                                    ...
                                    struct audio_stream_in *stream)
{
    struct audio_device *adev = (struct audio_device *)dev;
    struct stream_in *in = (struct stream_in*)stream;
    if (!in->stream_opened) goto no_need_to_close;
    ALOGVV("%s : Enter", __func__);
                     ...
                     hw_device_t **device)
{
    ...
    *device = NULL;
    ...
    adev->prev_mode = AUDIO_MODE_NORMAL;
    adev->active_input = NULL;
    adev->primary_output = NULL;

    adev->voice.volume = 100;
    adev->voice.bluetooth_nrec = true;
    adev->voice.in_call = false;
    adev->voice.bluetooth_wb = false;
